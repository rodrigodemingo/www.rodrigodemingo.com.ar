...
// set the proper jQuery identifier
$ = jQuery;
...
//                );
                // check if function exists [4.6]
                if(self.editor.matchHighlight) self.editor.matchHighlight("CodeMirror-searching");
            },
            onChange: function(){
                self.editor.save();
                self.onChange();
        ...
        // since 4.5
        self.range = function() {
            return { from: self.editor.getCursor(true), to: self.editor.getCursor(false) };
        }
