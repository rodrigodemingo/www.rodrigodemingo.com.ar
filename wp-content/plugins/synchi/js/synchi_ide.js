...
function synchiIDE_tab(file,focus) {
    ...
    self.editor = $('<div rel="'+self.id+'" class="synchi_ide_file" style="display:none"></div>');    
...
function synchiIDE_editor_action(action) {
    ...
    switch(action) {
        ...
        case 'comment':
            if(!tab) return;
            if($.inArray(tab.extension, ['php','js','css']) < 0) return;
...
function synchiIDE_updateDimensions() {
    if(synchiIDE_fullscreen) {
        ...
        $('.synchi_ide_file').css('max-width',$("#synchi_ide_editor").width() - SYNCHI_IDE_WIDTH_DIFFERENCE);
    }
    else {
        $('.synchi_ide_file').css('max-width',SYNCHI_EDITOR_DEFAULT_MAX_WIDTH+'px');
    }
}
...
function synchiIDE_sidebar_select(link,closeMenu) {
    ...
    $("#synchi_ide_sidebar_filesize").text(synchi_labels['File size'] + ': '+link.attr('filesize'));
...
function synchiIDE_initTabs() {
    ...
    for(var i=0; i<synchi_serialized_tabs.length; i++) {
        ...
        var focus = (i == 0) ? 'true' : 'false';
        setTimeout('synchiIDE_openTab("'+tab+'",'+focus+')',i*300);
...
function synchiIDE_init() {
    ...
    synchi_call('get_ide', {editor_mode: synchi_editor_mode}, function(response){
        ...
        synchi_hideMessage(function(){
            ...
            SYNCHI_EDITOR_DEFAULT_MAX_WIDTH = $("#synchi_ide_editor").width() - SYNCHI_IDE_WIDTH_DIFFERENCE;
