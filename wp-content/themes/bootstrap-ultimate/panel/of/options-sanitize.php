	...
	$multicheck_defaults = array(
		...
		foreach( $option['options'] as $key => $value ) {
			...
			foreach( $input as $selected_val ) {
				if($key == $selected_val ) {
					/* This supposedly checks if input_key ("pancakes") matches any of the $option['options']['key']. That's fine, but the interface example is wrong-misleading.
					$multicheck_defaults = array(
							'one' => '1',
							'five' => '1'
						);
					In this case, 1 is checked for match in $option['options']['key']s, so the default interface example should be :
					$multicheck_defaults = array("one","five");		
					*/
					$output[$selected_val] = '1';
...
// _eo-of-mod: disable font face sanitation
